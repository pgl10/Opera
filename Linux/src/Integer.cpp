...
//
// Inc/Dec -- postfix: a++
Integer	Integer::operator ++(int) {Integer r=_x;mpz_add_ui(_x,_x,1); return r;}
Integer	Integer::operator --(int) {Integer r=_x;mpz_sub_ui(_x,_x,1); return r;}
//
...
void	Int2gmp(Integer i, mpz_t n) {char *a; a=(char*)malloc(mpz_sizeinbase(i._x,i.base)*sizeof(char)); 
                 mpz_get_str(a, i.base, i._x ); mpz_set_str(n, a, i.base); free(a);}
...
bool	isprime(Integer a) 	{ return mpz_probab_prime_p(a._x, 25)!=0; }
//bool	issquare(Integer a)	{ return (bool) mpz_perfect_square_p(a._x); }
bool	issquare(Integer a)	{ return mpz_perfect_square_p(a._x)!=0; }
...
/***********************************************************************/
#define MRAND_MAX (RAND_MAX*(RAND_MAX + 2))  // (2^30-1)=(2^15-1)(2^15+1)
unsigned int mrand (void) {                  // comme rand() mais sur 30 bits
    return rand () * ((unsigned int)RAND_MAX + 1) + rand ();
}
...
*/
	srand((long)time(&t));	    // set seed
	rr = (mrand()&127) + 1;	    // get a random number
	siz = (log2(a)+1)*3;
